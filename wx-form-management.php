...
class wx_form_management {
  ...
  function wx_build_link($formUrl, $content, $button = array('color'=>'FFF','background'=>'7BB045')) {
    ...
    return '<div class="holds-wc-button"><a class="wx-button" target="_blank" href="'.$formUrl.'">'.$content.'</a></div>';  
  }
  ...
  function wx_build_embed($formUrl, $content) 
  {
    ...
    return '<div class="holds-wx-embed"><p class="notice">This window is secured by 256 bit encryption on a PCI compliant network. <a href="'.$directURL.'" target="_blank">Click here</a> to view this window in its own page.</p><iframe class="wx-embed" src="'.$formUrl.'" width="100%" scrolling="no"></iframe></div>';
  }
  ...
  function wx_build_modal($formUrl, $content, $button = array('color'=>'FFF','background'=>'7BB045'))
  {
    ...
    wp_enqueue_style('wx-form-management', plugin_dir_url( __FILE__ ) . 'wx-form-management-styles.php?color='.$button['color'].'&background='.$button['background'], null, '1.0');
    return '<div class="holds-wc-button"><a class="wx-button thickbox" href="'.$formUrl.'?TB_iframe=true&width=800&height=550">'.$content.'</a></div>';
  }
